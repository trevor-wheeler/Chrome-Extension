// List of hosts to run this script on
const hostnames = [
  'www.youtube.com',
  'www.instagram.com',
  'www.facebook.com'
...
function main() {
  // Block content for the the active website
  switch (window.location.hostname) {
    case 'www.youtube.com':
      // Element Variables
      const yt_nav = document.getElementById('sections')?.children[0]?.lastElementChild?.children[1];
      const yt_collapsedNav = document.getElementsByTagName('ytd-mini-guide-entry-renderer')[1];
      const yt_homepageShorts = document.getElementsByTagName('ytd-rich-section-renderer');
      const yt_sidebarShorts = document.getElementsByTagName('ytd-reel-shelf-renderer');
      const yt_searchShortsResults = Array.from(document.querySelectorAll('badge-shape[aria-label="Shorts"]'))
      .map(result => result.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement);
      // Block content
      blockContent('yt-shorts-page', [yt_nav, yt_collapsedNav]);
      blockContent('yt-shorts-home', [yt_homepageShorts, yt_sidebarShorts]);
      blockContent('yt-shorts-search', [yt_searchShortsResults]);
      break;
    case 'www.instagram.com':
      // Element Variables
      const ig_reelsPage = document.querySelector('.x1i10hfl[href="/reels/"]')?.parentElement.parentElement.parentElement;
      const ig_explorePage = document.querySelector('.x1i10hfl[href="/explore/"]')?.parentElement.parentElement.parentElement;
      const ig_reelsInDMs = Array.from(document.querySelectorAll('.x1ey2m1c.xds687c.x17qophe.x10l6tqk.x13vifvy.x6m44yg'))
      .map(result => result.parentElement.parentElement.parentElement);
      const ig_reelsInDMsReplies = Array.from(document.querySelectorAll('.x1tlxs6b.x1g8br2z.x1gn5b1j.x230xth.x6ikm8r.x10wlt62.x19g9edo.x1n2onr6'))
      .map(result => result.parentElement.parentElement.parentElement.parentElement);
      // Block content
      blockContent('ig-reel-page', [ig_reelsPage]);
      blockContent('ig-explore-page', [ig_explorePage]);
      blurContent('ig-reels-dms', [ig_reelsInDMs, ig_reelsInDMsReplies]);
      break;
    case 'www.facebook.com':
      // Element Variables
      const fb_reelsPage = document.querySelector('a[href="https://www.facebook.com/reel/?s=bookmark"]')?.parentElement.parentElement;
      const fb_ReelsPage2 = document.querySelector('a[href="/reel/?s=bookmark"]')?.parentElement
      const fb_reelsHome = document.querySelector('.x78zum5.x1q0g3np.x1qughib.xz9dl7a.xn6708d.x1120s5i.x1ye3gou')?.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement;
      const fb_reelsInDMs = document.querySelectorAll('.html-div.xdj266r.x11i5rnm.xat24cr.x1mh8g0r.xexx8yu.x4uap5.x18d9i69.xkhd6sd.x193iq5w.x1n2onr6.x1vjfegm.xh8yej3');
      // Block content
      blockContent('fb-reels-home', [fb_reelsHome]);
      blockContent('fb-reel-page', [fb_reelsPage, fb_ReelsPage2]);
      blurContent('fb-reels-dms', [fb_reelsInDMs]);
      break;
  }
}
  ...
  chrome.storage.sync.get([option], data => {
    ...
    // If Error was thrown
      ...
      window.getDefaultState(option).then(state => {
        ...
        // Run blockContent function again
...
function blurContent(option, elements) {
  ...
  chrome.storage.sync.get([option], data => {
    try {
      ...
      // For each element that needs to be blurred/unblurred
      elements.forEach(element => {
        // If the element exists and it is a list/array
        if (element && typeof element.length === 'number' && element.length > 0) {
          // For each element in that array
    ...
    // If Error was thrown
      ...
      window.getDefaultState(option).then(state => {
        chrome.storage.sync.set({ [option]: state });
        // Run blurContent function again
...
function handleBlur(optionState, element, option) {
  // Ensure element is positioned relatively
  if (getComputedStyle(element).position === 'static') {
    element.style.position = 'relative';
  }

  // If the option is enabled in local storage
  if (optionState) {
...
// If the host name of the current page is in the list of allowed hostnames
